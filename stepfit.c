...
{
    volatile long double sigma1, sigma2, a, b;
    volatile long double t1 = maxlength/2.0 * (1.0 + tanh(p[2])); //constrain t1 to (0, maxlength)
    volatile long double t2 = maxlength/2.0 * (1.0 + tanh(p[5])); //constrint t2 to (0, maxlength)
    volatile long double fitval = sign*maxbaseline/2.0 * (1.0 + tanh(p[0])); //constrain baseline to sign * (0, maxbaseline)
    if (time > t1)
    {
...
void step_response(event *current, volatile long double risetime, int64_t maxiters, volatile long double minstep)
{
    ...
    if (current->type == STEPRESPONSE)
    {
        ...
        volatile long double *time;
        time = calloc_and_check(length, sizeof(volatile long double), "cannot allocate stepfit time array"); //time array
        time_array(time, length);
        int64_t n = 7; // number of parameters in model function f
        volatile long double par[n];  // parameter array


        volatile long double maxsignal = signal_max(current->signal, current->length + current->padding_before + current->padding_after);
        volatile long double minsignal = signal_min(current->signal, current->length + current->padding_before + current->padding_after);
        volatile long double baseline = signal_average(current->signal,current->padding_before);
        int sign = signum(baseline);
        int64_t start = current->padding_before;
        int64_t end = current->length + current->padding_before;
        volatile long double stepguess = 0.66 * sign * (minsignal - maxsignal);
        volatile long double maxlength = (volatile long double) length;
        volatile long double maxstep = d_abs(maxsignal - minsignal);
        volatile long double maxbaseline = my_max(d_abs(maxsignal),d_abs(minsignal));
        ...
        volatile long double i0 = sign*maxbaseline/2.0 * (1.0 + tanh(par[0]));
        volatile long double a = sign*maxstep/2.0 * (1.0 + tanh(par[1]));
        int64_t u1 = maxlength/2.0 * (1.0 + tanh(par[2]));
        volatile long double rc1 = risetime * exp(par[3]);
        volatile long double b = sign*maxstep/2.0 * (1.0 + tanh(par[4]));
        int64_t u2 = maxlength/2.0 * (1.0 + tanh(par[5]));
        volatile long double rc2 = risetime * exp(par[6]);
        volatile long double residual = 0;
